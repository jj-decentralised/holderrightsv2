...
function seedIfEmpty() {
  ...
  if (items.length === 0) {
    ...
    items = [
      // Twitter
      { _id: "t1", type: "twitter", title: "Virtuals ecosystem break down", status: "pitch", format: "thread", createdAt: now, updatedAt: now },
      ...
      // Editorial
      { _id: "e1", type: "editorial", title: "ZkSync Thesis", status: "drafting", assignee: "Saurabh", dueDate: now - 24 * day, category: "organic", createdAt: now, updatedAt: now },
      { _id: "e2", type: "editorial", title: "Semiliquid", status: "assigned", assignee: "Joel", dueDate: now - 18 * day, category: "organic", createdAt: now, updatedAt: now },
      { _id: "e3", type: "editorial", title: "Holder rights", status: "assigned", assignee: "Joel", dueDate: now - 10 * day, category: "organic", createdAt: now, updatedAt: now },
      { _id: "e4", type: "editorial", title: "Drift Thesis", status: "assigned", assignee: "Joel", dueDate: now - 9 * day, category: "organic", createdAt: now, updatedAt: now },
      { _id: "e5", type: "editorial", title: "LiFi", status: "review", assignee: "Saurabh", dueDate: now - 8 * day, category: "sponsored", createdAt: now, updatedAt: now },
      { _id: "e6", type: "editorial", title: "LayerZero", status: "assigned", assignee: "Joel", dueDate: now - 4 * day, category: "sponsored", createdAt: now, updatedAt: now },
      { _id: "e7", type: "editorial", title: "Crypto revenue analysis", status: "drafting", assignee: "Saurabh", dueDate: now - 1 * day, category: "organic", createdAt: now, updatedAt: now },
      ...
      { _id: "e9", type: "editorial", title: "Ronin Deep Dive", status: "drafting", assignee: "Joel", category: "sponsored", createdAt: now, updatedAt: now },
...
export function useOverdue(): ContentItem[] {
  const all = useItems();
  const now = Date.now();
  return all.filter((i) => i.dueDate && i.dueDate < now && i.status !== "published" && i.status !== "delivered");
}
...
export function createItem(data: Omit<ContentItem, "_id" | "createdAt" | "updatedAt">) {
  const now = Date.now();
  items = [...items, { ...data, _id: newId(), createdAt: now, updatedAt: now } as ContentItem];
  persist(); emit();
}

export function updateItem(id: string, updates: Partial<ContentItem>) {
  items = items.map((i) => i._id === id ? { ...i, ...updates, updatedAt: Date.now() } : i);
  persist(); emit();
}
